...
func Exec(file *os.File, session *mgo.Session, Wg *sync.WaitGroup, args string,logfile *os.File) {
	...
	LogInfo.Println("Directory changed to new directory "+"/tmp")
	...
	LogInfo.Println("Directory changed to old directory "+ oldWd)
	
	os.RemoveAll("../tmp/"+taskname+"/")
